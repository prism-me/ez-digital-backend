...
class ProjectController extends Controller
{
    ...
    public function create_project(Request $request){
        ...
        if($response){
            if($this->add_search_engine($response->site_id, $request->search_engine_id)){
                ...
                $project->search_engine_id = $request->search_engine_id;
                $project->save();

                return response()->json([ 'success' => 'Project added successfully', "site_id" => $response->site_id], 200);

            }else{
                return response()->json([ 'error' => 'Invalid search engine!'], 400);
    ...
    public function add_search_engine($site_id, $search_engine_id){
        $data = [
            "search_engine_id" => $search_engine_id
        ];
        $end_point = "sites/".$site_id."/search-engines";
        $response = $this->SeRankingApi($data, $end_point);

        if($response){
            return 1;
            Project::where('site_id', $site_id)->update(['search_engine_id' => $search_engine_id]);
